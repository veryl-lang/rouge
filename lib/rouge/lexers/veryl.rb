...
module Rouge
  module Lexers
    class Veryl < RegexLexer
      ...
      def self.keywords
        @keywords ||= Set.new %w(
          embed enum function impl include interface modport module package proto pub struct union unsafe
      ...
      def self.keywords_type
        @keywords_type ||= Set.new %w(
          bit bbool lbool clock clock_posedge clock_negedge f32 f64 i8 i16 i32 i64 logic reset reset_async_high reset_async_low reset_sync_high reset_sync_low signed string tri p8 p16 p32 p64 u8 u16 u32 u64
        )
      end
